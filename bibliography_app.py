...
tab1, tab2, tab3, tab4 = st.tabs(["Authors", "Co-authors", "Forrester Number", "Forrester's Co-authors"])
...
with tab3:
    ...
    # Check if reference author exists
    ...
    if reference_in_graph is None:
        ...
        # Show Forrester's info
        info = G.nodes[reference_in_graph]
        st.caption(f"**{reference_in_graph}** has **{info.get('num_papers', 0)}** papers and **{info.get('num_coauthors', 0)}** direct co-authors in the database.")
        ...
        if author_query_tab3:
            ...
            if not candidates:
                ...
                author_names = [name for name, score in candidates]
                selected_author_tab3 = st.radio("Select an author:", options=author_names, key="forrester_select")
                            ...
                            if nx.has_path(G, selected_author_tab3, reference_in_graph):
                                ...
                                for path in paths_to_show:
                                    for j in range(len(path) - 1):
                                        ...
                                        if "papers" in G[a][b]:
                                            path_graph[a][b]["papers"] = G[a][b]["papers"]
                                
                                for node in path_graph.nodes():
                                    dist = nx.shortest_path_length(path_graph, selected_author_tab3, node)
                                    path_graph.nodes[node]["level"] = dist
                                    if node in G.nodes:
                                        path_graph.nodes[node].update(G.nodes[node])
                                ...
                                if path_graph.number_of_nodes() > 0:
                                    ...
                                    for n in path_graph.nodes():
                                        ...
                                        if n == selected_author_tab3:
                                            node_colors.append("#d62828")
                                            node_sizes.append(50)
                                        elif n == reference_in_graph:
                                    ...
                                    legend_html = (
                                        ...
                                        '<span style="color:#555; font-size:13px;">Selected author</span></span>'
                                        '<span style="display:inline-flex; align-items:center;">'
                                        '<span style="width:12px; height:12px; border-radius:50%; background-color:#f4a261; margin-right:5px;"></span>'
                                ...
                                st.warning(f"⚠️ **{selected_author_tab3}** is not connected to {REFERENCE_AUTHOR} in the co-author network.")
...
# =============================================================================
# Tab 4: Forrester's Co-authors
# =============================================================================

with tab4:
    st.header("Forrester's Direct Co-authors")
    
    st.markdown(
        f"""
        This page lists everyone who has co-authored a paper with **{REFERENCE_AUTHOR}** in the dataset.
        These are the **1st degree connections** — people with a Forrester Number of 1.
        
        *This list can help identify data quality issues and see who is already connected before adding advisor relationships.*
        """
    )
    
    # Check if Forrester exists
    forrester_in_graph = None
    for node in G.nodes():
        if normalize_author_name(REFERENCE_AUTHOR) == normalize_author_name(node):
            forrester_in_graph = node
            break
    
    if forrester_in_graph is None:
        st.error(f"**{REFERENCE_AUTHOR}** not found in the co-author network.")
    else:
        # Get all neighbors
        neighbors = list(G.neighbors(forrester_in_graph))
        
        st.success(f"**{forrester_in_graph}** has **{len(neighbors)}** direct co-authors in the dataset.")
        
        # Build table with details
        coauthor_data = []
        for name in neighbors:
            weight = G[forrester_in_graph][name].get('weight', 1)
            papers = G[forrester_in_graph][name].get('papers', [])
            
            # Get paper titles and years
            paper_details = []
            for p in papers:
                title = p.get('title', 'Unknown')
                year = p.get('year', '')
                year_str = f" ({int(year)})" if pd.notna(year) else ""
                paper_details.append(f"{title}{year_str}")
            
            # Get author info
            num_papers = G.nodes[name].get('num_papers', 0) if name in G.nodes else 0
            country = G.nodes[name].get('country', '') if name in G.nodes else ''
            org = G.nodes[name].get('organization', '') if name in G.nodes else ''
            
            # Flag potential issues
            issues = []
            if len(name.split()) == 1:
                issues.append("Single name only")
            if len(name) <= 3:
                issues.append("Very short name")
            if any(c.isdigit() for c in name):
                issues.append("Contains numbers")
            if name.lower() in ['unknown', 'anonymous']:
                issues.append("Unknown/Anonymous")
            ...
            coauthor_data.append({
                ...
                'Paper Titles': '; '.join(paper_details),
                'Potential Issues': ', '.join(issues) if issues else ''
            })
        
        # Sort by shared papers descending
        coauthor_df = pd.DataFrame(coauthor_data)
        coauthor_df = coauthor_df.sort_values('Shared Papers', ascending=False)
        coauthor_df.index = range(1, len(coauthor_df) + 1)
        
        # Summary stats
        col1, col2, col3 = st.columns(3)
        with col1:
            issues_count = sum(1 for d in coauthor_data if d['Potential Issues'])
            st.metric("Co-authors with Potential Issues", issues_count)
        with col2:
            single_paper = sum(1 for d in coauthor_data if d['Shared Papers'] == 1)
            st.metric("Single Paper Collaborations", single_paper)
        with col3:
            multi_paper = sum(1 for d in coauthor_data if d['Shared Papers'] > 1)
            st.metric("Multi-Paper Collaborations", multi_paper)
        
        st.divider()
        
        # Filter options
        show_issues_only = st.checkbox("Show only entries with potential issues", key="show_issues")
        
        if show_issues_only:
            display_df = coauthor_df[coauthor_df['Potential Issues'] != '']
        else:
            display_df = coauthor_df
        
        st.subheader(f"Co-author List ({len(display_df)} shown)")
        st.caption("Double-click a cell to read full text. The 'Potential Issues' column flags possible data quality problems.")
        
        st.dataframe(
            display_df,
            use_container_width=True,
            column_config={
                "Paper Titles": st.column_config.TextColumn(
                    "Paper Titles",
                    width="large"
                )
            }
        ...
        csv = coauthor_df.to_csv(index=True)
        st.download_button(
            label="Download full list as CSV",
            data=csv,
            file_name="forrester_coauthors.csv",
            mime="text/csv"
        )
